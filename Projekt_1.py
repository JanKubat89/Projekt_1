...
# ----------------------
# TODO   Vytvorit promennou  reg_users' jako dictationary
...
# Neprisel jsem na to, jak jinak tento optimalnejsi zapis dostat na 75
# znaku na radek bez toho aniz bych zkratil nazev promenne
# 'registered_users' na 'reg_users'
if username in reg_users.keys() and password == reg_users[username]:
    print(f'''----------------------------------------
Welcome to the app, {username.capitalize()}.
...
# Mene optimalnejsi kod, ktery funguje uplne stejne s puvodnim nazvem
# promenne 'registered_users'
"""
...
if int(text_selection_value) not in range(1, 4):
...
selected_text = TEXTS[int(text_selection_value) - 1]
cleaned_text = list()
...
for word_in_text in cleaned_text:
...
centralization_of_text = (
    int((max(word_length.values()) + 2 - 10) / 2) * ' '
    )

print(f'''----------------------------------------
LEN|{centralization_of_text}OCCURENCES{centralization_of_text}|NR.
----------------------------------------''')
...
if int(max(word_length.values())) % 2 != 0:
    for key, value in sorted(word_length.items()):
        max_occurences = ((max(word_length.values()) - value) * ' ')
        if key <= 9 and value <= 9:
            print(f"  {key}|{value*'*'}{max_occurences} | {value}")
